    ...
    function find(){
        ...
        if(memory.length === 0){
            ...
            for(let i = 0; i < memory.length; i++){
                if(compareArrays(Array.from(arguments), memory[i].args) === true && memory.length > 0){
                    let result = memory[i].result;
                    return result;
                }
                else if(compareArrays(Array.from(arguments), memory[i].args) === false){
